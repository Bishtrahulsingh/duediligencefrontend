...
interface PolishedAnswer {
  ...
  summary?: string           // "Insufficient data" path
}

/** Top-level shape returned by the backend */
...
/** Safely parse the backend payload into AnalysisResponse */
function parseResponse(raw: unknown): AnalysisResponse | null {
  if (!raw || typeof raw !== 'object') return null
  const r = raw as Record<string, unknown>

  // Backend wraps in { response: ... }
  ...
  return {
    ...
    evidence: (inner.evidence as AnalysisResponse['evidence']) ?? {
      supporting: null,
      contradicting: null,
    },
  }
...
function ScoreBar({
  ...
  const color =
    value >= threshold         ? 'bg-dl-green'  :
    value >= threshold - 0.2   ? 'bg-dl-yellow' : 'bg-dl-red'
  const textColor =
    value >= threshold         ? 'text-dl-green'  :
    value >= threshold - 0.2   ? 'text-dl-yellow' : 'text-dl-red'
...
export default function AnalysePage() {
  ...
  const currentEntry: TickerEntry | undefined =
    tickerMap.find((e) => e.ticker === selectedTicker)
    ...
    <>
      ...
      <div className="md:p-7 md:flex gap-5">
        ...
        <div className="py-5 flex-1 flex flex-col bg-dl-surface border border-dl-border rounded-xl">
          ...
          <div
            ref={resultRef}
            className="flex-1 p-5 overflow-y-auto flex flex-col gap-5 min-h-[320px] max-h-[640px]"
          >
              ...
              <div className="flex items-center gap-2.5 px-3.5 py-2.5 bg-[rgba(232,160,32,0.06)] border border-[rgba(232,160,32,0.2)] rounded-lg font-mono text-[11px] text-dl-amber animate-fade-in">
                <div className="flex gap-1">
                  {[0, 1, 2].map((i) => (
                    <span
                      key={i}
                      className="w-1.5 h-1.5 rounded-full bg-dl-amber animate-blink"
            ...
            {result && pa && (
              <div className="flex flex-col gap-5 animate-fade-in">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex flex-col gap-2">
                      {pa.key_risks.map((r, i) => (
                        <div
                          key={i}
                          className="flex items-start gap-3 px-3.5 py-3 bg-dl-surface2 border border-dl-border rounded-lg hover:border-dl-border2 transition-colors"
                        >
                          <span className={cn(
                            'inline-flex px-2 py-0.5 rounded-full font-mono text-[9px] font-medium shrink-0 mt-0.5',
                            RISK_COLOR[r.severity?.toLowerCase()] ?? RISK_COLOR.medium,
                          )}>
                          ...
                          <div className="flex flex-col gap-1.5">
                            ...
                            {r.evidence && (
                              <p className="font-mono text-[10px] text-dl-text3 leading-relaxed">
                                <span className="text-dl-amber mr-1">Evidence:</span>{r.evidence}
                              </p>
                            )}
                          </div>
                        </div>
                    ...
                    <div className="flex flex-col gap-2">
                      {pa.open_questions.map((q, i) => {
                        ...
                        return (
                          <div
                            key={i}
                            className="flex items-start gap-2.5 px-3.5 py-3 bg-dl-surface2 border border-dl-border rounded-lg"
                          >
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex flex-col gap-1.5">
                      {result.issues.map((issue, i) => (
                        <div
                          key={i}
                          className="flex items-start gap-2.5 px-3.5 py-2.5 bg-[rgba(224,85,85,0.06)] border border-[rgba(224,85,85,0.15)] rounded-lg font-mono text-[11px] text-dl-red leading-relaxed"
                        >
                ...
                {(result.evidence?.supporting || result.evidence?.contradicting) && (
                  <div className="flex flex-col gap-2">
                    <SectionLabel icon={<BookOpen size={12} />} label="Context Evidence" />
                    <div className="flex flex-col gap-2">
                      {result.evidence.supporting && (
                        <div className="px-3.5 py-3 bg-[rgba(72,196,122,0.06)] border border-[rgba(72,196,122,0.2)] rounded-lg">
                          <p className="font-mono text-[9px] uppercase tracking-[0.8px] text-dl-green mb-1.5">
                            Supporting
                          </p>
                          <p className="font-mono text-[11px] text-dl-text2 leading-relaxed">
                            {result.evidence.supporting}
                          </p>
                        </div>
                      )}
                      {result.evidence.contradicting && (
                        <div className="px-3.5 py-3 bg-[rgba(224,85,85,0.06)] border border-[rgba(224,85,85,0.15)] rounded-lg">
                          <p className="font-mono text-[9px] uppercase tracking-[0.8px] text-dl-red mb-1.5">
                            Contradicting
                          </p>
                          <p className="font-mono text-[11px] text-dl-text2 leading-relaxed">
                            {result.evidence.contradicting}
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="px-4 py-4 bg-dl-surface2 border border-dl-border rounded-lg flex flex-col gap-3.5">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-mono text-[9px] text-dl-text3">
                        thresholds · faithfulness ≥ 0.7 · relevance ≥ 0.7
                      </span>
                    </div>
                ...
                {/* 7 · Confidence (analyst-level, optional) */}
                {pa.confidence != null && (
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex items-center gap-4 px-4 py-3.5 bg-dl-surface2 border border-dl-border rounded-lg">
                      <span className="font-serif text-[36px] font-light text-dl-amber leading-none">
                        {typeof pa.confidence === 'number'
                          ? pa.confidence.toFixed(2)
                          : pa.confidence}
                      </span>
                      <div className="flex-1">
                        <div className="font-mono text-[9px] text-dl-text3 uppercase tracking-[1px] mb-1.5">
                          Context Coverage
                        </div>
                        <div className="h-1.5 bg-dl-surface3 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${typeof pa.confidence === 'number'
                                ? pa.confidence * 100 : 80}%`,
                            }}
                          />
                        </div>
            ...
            {!running && !hasResult && !error && (
              <div className="flex-1 flex flex-col items-center justify-center text-center py-12 text-dl-text3">
                <svg
                  ...
                  stroke="currentColor" strokeWidth={1.5} className="mb-3 opacity-30"
                >
                  <polyline points="22 12 18 12 15 21 9 3 6 12 2 12" />
                </svg>
          ...
          <div className="border-t border-dl-border px-5 py-4 flex flex-col gap-3">
            ...
            <div className="flex gap-2 items-end">
              <textarea
                ...
                placeholder={
                  currentEntry
                    ? `Ask about ${currentEntry.name}${selectedYear ? ` FY${selectedYear}` : ''}…`
              ...
              <>
                <div className="flex items-center gap-3 mb-3">
                  <div className={cn(
                    ...
                    AVATAR_COLORS[tickerMap.indexOf(currentEntry) % AVATAR_COLORS.length],
                  )}>
                  ...
                  <div>
                    ...
                    <div className="font-mono text-[10px] text-dl-text3 mt-0.5">
                      {currentEntry.ticker}{selectedYear ? ` · FY${selectedYear}` : ''}
                    </div>
                  </div>
                </div>

                <div className="flex flex-col gap-1.5">
                  <div className="font-mono text-[9px] uppercase tracking-[1px] text-dl-text3">
                    Available years
                  </div>
                  {yearsLoading ? (
                    <div className="flex gap-1.5 animate-pulse">
                      {[0, 1, 2].map((i) => (
                        <div key={i} className="w-10 h-5 rounded bg-dl-surface3" />
                      ))}
                    </div>
                  ) : availableYears.length === 0 ? (
                    <div className="font-mono text-[10px] text-dl-text3">
                      No documents yet —{' '}
                      <a href="/dashboard/documents" className="text-dl-amber hover:underline">
                        ingest one →
                      </a>
                    </div>
                  ) : (
                    ...
                    <div className="flex flex-col items-center shrink-0">
                      ...
                      {i < PIPELINE_STEPS.length - 1 && (
                        <div
                          className={cn(
                            'flex-1 w-px mt-1',
                            done ? 'bg-dl-green opacity-30' : 'bg-dl-border',
                          )}
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        entry.ticker === selectedTicker
                          ? 'text-dl-amber'
                          : 'text-dl-text2 hover:text-dl-text',
                      )}
                    >
                      <div className={cn(
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="font-mono text-[10px] text-dl-text3">
                          ...
                          {cachedYears.length > 0 && (
                            <span className="ml-1">
                              · {cachedYears.length} yr{cachedYears.length !== 1 ? 's' : ''}
                            </span>
                          )}
                        </div>
                      </div>
                      {entry.ticker === selectedTicker && (
                        <span className="w-1.5 h-1.5 rounded-full bg-dl-amber shrink-0" />
                      )}
                    </button>
                  )
                })}
